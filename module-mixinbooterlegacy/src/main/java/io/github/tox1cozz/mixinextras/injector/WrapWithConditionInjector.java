...
public class WrapWithConditionInjector extends Injector {
    ...
    protected void inject(Target target, InjectionNode node) {
        checkTargetIsLogicallyVoid(target, node);
        checkTargetModifiers(target, false);
        wrapTargetWithCondition(target, node);
    }
    ...
    private void invokeHandler(Target target, Type returnType, Type[] originalArgTypes, Type[] currentArgTypes, boolean isVirtualRedirect, InsnList before, InsnList after) {
        ...
        validateParams(handler, Type.BOOLEAN_TYPE, originalArgTypes);

        int[] argMap = storeArgs(target, currentArgTypes, before, 0);
        ...
        invokeHandlerWithArgs(methodArgs, before, handlerArgMap);
        ...
        pushArgs(currentArgTypes, before, argMap, 0, argMap.length);
        // Target instruction will be here
        after.add(new JumpInsnNode(Opcodes.GOTO, afterDummy));
        after.add(afterOperation);
        if (returnType != Type.VOID_TYPE) {
            after.add(new InsnNode(getDummyOpcodeForType(returnType)));
        }
